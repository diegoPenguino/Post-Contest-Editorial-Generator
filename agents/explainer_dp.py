...
def explainer_dp_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    report_progress(state, "Phase: Explaining the dynamic programming approach")
    ...
    logger.info(
        "DP explainer prompt prepared: chars=%d model=%s temperature=%s",
        len(prompt),
    ...
    explanation = log_llm_response("explainer_dp_node", response)
